...
function routeTo(diagram, key, graphId) {
	...
	if (routeList == null || routeList == "undefined") {
		if (localStorage.routeList == null || localStorage.routeList == "null"
				|| localStorage.routeList == "undefined")
			routeList = new Array();
		else {
			// console.log(" router...
			// localStorage.routeList="+JSON.stringify(localStorage.routeList));
	...
	// // console.log(" router... currentStepInRouteList=
	...
	if (diagram == null || diagram == "undefined") {
		...
		// 如果当前步骤数在记录范围，就跳转
		// // console.log(" router... ");
		if (currentStepInRouteList < routeList.length
			...
			// console.log(" router... 执行前进后退后, _routeID="+_routeID+"
			// diagram="+diagram);
			// console.log(" router... 执行前进后退后，currentStepInRouteList=" +
			// currentStepInRouteList
			// + " routeList.length=" + routeList.length);
		} else {
			alert("已经到头了。");
		...
		if (currentStepInRouteList == routeList.length - 1) {
			...
			// 删除当前步后面的记录
			// console.log(" router... 前进后退后，中间点了正常跳转功能。前：
			// currentStepInRouteList=" + currentStepInRouteList
			// + " routeList.length=" + routeList.length);
			// currentStepInRouteList = routeList.length - 1;
			routeList.splice(currentStepInRouteList + 1, routeList.length
					- currentStepInRouteList - 1);
			// console.log(" router... 前进后退后，中间点了正常跳转功能。后：
			// currentStepInRouteList=" + currentStepInRouteList
			// + " routeList.length=" + routeList.length);
		}
		// 传递参数
		...
		// console.log(" router... 执行正常跳转后，currentStepInRouteList=" +
		// currentStepInRouteList
		// + " routeList.length=" + routeList.length);
	}
	...
	// 查询框
	var serachbox_topToolsBar = document
			.getElementById("serachbox_topToolsBar");
	// console.log(" router... switch前 , _routeID="+_routeID+"
	// diagram="+diagram);
	// console.log(" router... switch前 ，currentStepInRouteList=" +
	// currentStepInRouteList
	// + " routeList.length=" + routeList.length);
	...
	case "diagramDetail".toLowerCase():
		...
		_diagramShowKey = key;
		_graphId = graphId;
		// console.log(" router... =====调试图形svg未加载问题 ===== router.js ->
		// diagramDetail");
		// console.log(" router... =====调试图形svg未加载问题 ===== router.js ->
		// diagramDetail");
		...
		for (var indi = 0; indi < itemD.length; indi++) {
			...
			if (itemDI.name != tags[inditem]) {
				...
				itemDIV.setAttribute('onclick', 'routeTo(\'' + itemDI.diagram
						+ '\',' + itemDI._galleryKey + ',' + itemDI._routeID
						+ ')');
...
function stepBackward() {
	if (currentStepInRouteList == null || currentStepInRouteList == "undefined"
		...
		if (localStorage.currentStepInRouteList == null
			...
			// "+localStorage.currentStepInRouteList);
			currentStepInRouteList = parseInt(localStorage.currentStepInRouteList);
		}
	}
	currentStepInRouteList--;
	// console.log(" router... stepBackward-> currentStepInRouteList=" +
	// currentStepInRouteList + " routeList.length="+ routeList.length);
	routeTo();
}
